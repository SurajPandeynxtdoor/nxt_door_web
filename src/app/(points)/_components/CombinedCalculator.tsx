          ...
          >
            <span className="inline-flex items-center gap-1">
              ...
              <span className="text-slate-600">
                ({num.format(Math.round(c.cardPoints))} pts <ArrowRight className="inline h-3 w-3" /> {c.partnerName})
              </span>
            </span>
  ...
  return (
    <div className="flex flex-col gap-3 rounded-xl border border-white/10 bg-white/5 p-4 sm:flex-row sm:items-center sm:justify-between">
      <div>
        ...
        {subtitle && <p className="text-xs capitalize text-slate-500">{subtitle}</p>}
      </div>
      <div className="flex items-center gap-2">
        <input
          type="number"
